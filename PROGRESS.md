...
| 4 | Dogfood on a real project | ⏳ in progress | — | Target: hexdeck itself. The board lives in `.kanban/` — it is the tracker now. Chunk 1 done: board init + migration + CI check. Chunk 2 done: the worker runs against the board, protocol in docs/worker.md. Chunk 3: the acceptance read |
...
| 4.3 | Dogfood acceptance: a human reads `board.md` and can answer "where is the project up to" without opening anything else. | ⏳ next |
...
- Phase 4 chunk 2 done: the worker runs against the board. `docs/worker.md` is the runbook — the full worker protocol: the loop (sync, close merged tickets, check feedback, one chunk, docs, PR, stop), how the board is used (pick before a chunk, comment at milestones, move to review at the end, the same-commit rule), and the hard rules. This run exercised it: the merge rule closed T-1 (PR #21 merged), the worker picked T-2, wrote the runbook with the board ops in the same commit, and moved T-2 to `review` — it waits for a human merge now. T-2 and T-3 went back to `todo` first: PR #21 named them, but it did not deliver them — a ticket is done when the PR that delivers it merges. Chunk 3 (the acceptance read) is next.
