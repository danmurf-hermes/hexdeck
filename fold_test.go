...
// ticket's Labels list, in the order the ops were added. A duplicate
// label is skipped with a warning.
func TestFoldLabelAdded(t *testing.T) {
	...
	writeOp(t, opsDir, 1, "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa", `{"schema":1,"opId":"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa","seq":1,"ts":"2026-08-20T14:00:00Z","actor":"claude-a","type":"ticket.created","ticket":"T-1","payload":{"title":"one"}}`)
	writeOp(t, opsDir, 2, "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb", `{"schema":1,"opId":"bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb","seq":2,"ts":"2026-08-20T14:01:00Z","actor":"claude-a","type":"ticket.label.added","ticket":"T-1","payload":{"label":"feature"}}`)
	...
	writeOp(t, opsDir, 4, "dddddddd-dddd-4ddd-8ddd-dddddddddddd", `{"schema":1,"opId":"dddddddd-dddd-4ddd-8ddd-dddddddddddd","seq":4,"ts":"2026-08-20T14:03:00Z","actor":"claude-a","type":"ticket.label.added","ticket":"T-1","payload":{"label":"feature"}}`)
	...
	t1 := state.Tickets["T-1"]
	if len(t1.Labels) != 2 || t1.Labels[0] != "feature" || t1.Labels[1] != "docs" {
		t.Errorf("T-1 labels = %v, want [feature docs]", t1.Labels)
	}
	if len(state.Warnings) != 1 {
		t.Fatalf("warnings = %v, want exactly 1 (the duplicate label)", state.Warnings)
	}
	if !strings.Contains(state.Warnings[0], "T-1 already has label feature") {
		t.Errorf("warning = %q", state.Warnings[0])
	}
}
...
func TestFoldLabelRemoved(t *testing.T) {
	...
	writeOp(t, opsDir, 1, "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa", `{"schema":1,"opId":"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa","seq":1,"ts":"2026-08-20T14:00:00Z","actor":"claude-a","type":"ticket.created","ticket":"T-1","payload":{"title":"one"}}`)
	writeOp(t, opsDir, 2, "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb", `{"schema":1,"opId":"bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb","seq":2,"ts":"2026-08-20T14:01:00Z","actor":"claude-a","type":"ticket.label.added","ticket":"T-1","payload":{"label":"feature"}}`)
	writeOp(t, opsDir, 3, "cccccccc-cccc-4ccc-8ccc-cccccccccccc", `{"schema":1,"opId":"cccccccc-cccc-4ccc-8ccc-cccccccccccc","seq":3,"ts":"2026-08-20T14:02:00Z","actor":"claude-a","type":"ticket.label.removed","ticket":"T-1","payload":{"label":"feature"}}`)
	// Removing a label that is not there must warn, not fail.
	writeOp(t, opsDir, 4, "dddddddd-dddd-4ddd-8ddd-dddddddddddd", `{"schema":1,"opId":"dddddddd-dddd-4ddd-8ddd-dddddddddddd","seq":4,"ts":"2026-08-20T14:03:00Z","actor":"claude-a","type":"ticket.label.removed","ticket":"T-1","payload":{"label":"feature"}}`)
	...
	if !strings.Contains(state.Warnings[0], "T-1 does not have label feature") {
		t.Errorf("warning = %q", state.Warnings[0])
	}
}

// TestFoldLabelMissingTicket checks the missing-ticket rule for labels:
// a label op about a ticket that was never created is skipped with a
// warning, never fatal.
func TestFoldLabelMissingTicket(t *testing.T) {
	...
	writeOp(t, opsDir, 1, "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa", `{"schema":1,"opId":"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa","seq":1,"ts":"2026-08-20T14:00:00Z","actor":"claude-a","type":"ticket.label.added","ticket":"T-99","payload":{"label":"feature"}}`)
